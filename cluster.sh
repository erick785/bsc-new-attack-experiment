...
# ---- topology: "pem|ip|start_idx|end_idx" --------------------------------
HOSTS=(
    "bsc-new-attack-1.pem|18.143.159.138|0|6"
    "bsc-new-attack-2.pem|3.90.13.14|7|13"
    "bsc-new-attack-3.pem|3.10.211.250|14|20"
)

# ---- attack experiment knobs (override via env) --------------------------
# Region of each host: host1=Singapore(sg), host2=US(us), host3=UK(uk).
SG_IP=$(IFS='|'; set -- ${HOSTS[0]}; echo "$2")
US_IP=$(IFS='|'; set -- ${HOSTS[1]}; echo "$2")
UK_IP=$(IFS='|'; set -- ${HOSTS[2]}; echo "$2")
ATTACK_SLOT="${ATTACK_SLOT:-}"      # first attack block height t; empty/0 => normal start (no attack)
ATTACK_PERIOD="${ATTACK_PERIOD:-168}" # blocks between repeated attacks = validators*turnLength (21*8).
                                    #   keeps every attack slot on the identical validator schedule
                                    #   (same US in-turn silenced, same b1/b2 eligible). 0 => single shot.
ATTACK_COUNT="${ATTACK_COUNT:-100}" # number of repeated attacks (samples) collected in one run.
LEAD_TIME_MS="${LEAD_TIME_MS:-60}"  # extra delay (ms) before sending b1 -> Singapore (UK nodes)
B1_NODE="${B1_NODE:-14}"            # UK node whose block (b1) targets Singapore
B2_NODE="${B2_NODE:-17}"            # UK node whose block (b2) targets US
...
# bsc embed files that the repo .gitignore (*.txt) drops; needed to build geth
EMBED_FILES=(
    "code/bsc/internal/era/eradl/checksums_mainnet.txt"
    "code/bsc/internal/era/eradl/checksums_sepolia.txt"
    "code/bsc/p2p/nat/stun-list.txt"
)
...
start_host() { # pem ip start end logfile attack_env  (run with & ; redirects its own subshell)
    ...
    run_ssh "$pem" "$ip" "
        ...
        echo '[make geth]'; cd code/bsc && make geth
...
case "${1:-}" in
    ...
    check)  cmd_check ;;
    *)
        echo "Usage: ./cluster.sh {stop|clean|start|set|status|result|check}"
