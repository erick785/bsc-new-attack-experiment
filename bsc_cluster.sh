...
extra_size=$((EXTRA_VALIDATOR_SIZE))
stateScheme="hash"
dbEngine="leveldb"
gcmode="full"
sleepBeforeStart=15
...
function create_validator() {
    ...
    # copy keys for extra validators (pre-funded in genesis, started later)
    for ((i = size; i < size + extra_size; i++)); do
        cp -r ${workspace}/keys/validator${i} ${workspace}/.local/
        cp -r ${workspace}/keys/bls${i} ${workspace}/.local/
    done
}
...
function prepare_config() {
    ...
    # pre-fund extra validators in genesis initHolders; do NOT add to validators.conf
    for ((i = size; i < size + extra_size; i++)); do
        for f in ${workspace}/.local/validator${i}/keystore/*; do
            cons_addr="0x$(cat ${f} | jq -r .address)"
            initHolders=${initHolders}","${cons_addr}
        ...
        targetDir=${workspace}/.local/node${i}
        mkdir -p ${targetDir}
        cp ${workspace}/keys/password.txt ${targetDir}/
        cp ${workspace}/.local/hardforkTime.txt ${targetDir}/
        mv ${workspace}/.local/bls${i}/bls ${targetDir}/ && rm -rf ${workspace}/.local/bls${i}
    done
...
function initNetwork() {
    ...
    # prepare geth dirs for extra validators (not started yet, just init chaindata)
    for ((i = size; i < size + extra_size; i++)); do
        mkdir ${workspace}/.local/node${i}/geth
        cp ${workspace}/keys/validator-nodekey${i} ${workspace}/.local/node${i}/geth/nodekey
        mv ${workspace}/.local/validator${i}/keystore ${workspace}/.local/node${i}/ && rm -rf ${workspace}/.local/validator${i}
    ...
    ${workspace}/bin/geth init-network --init.dir ${workspace}/.local --init.size=$((size + extra_size)) --config ${workspace}/config.toml ${init_extra_args} ${workspace}/genesis/genesis.json
    ...
    # init chaindata for extra validators
    for ((i = size; i < size + extra_size; i++)); do
        sed -i -e '/"<nil>"/d' ${workspace}/.local/node${i}/config.toml
        initLog=${workspace}/.local/node${i}/init.log
        ${workspace}/bin/geth --datadir ${workspace}/.local/node${i} init --state.scheme path --db.engine pebble ${workspace}/genesis/genesis.json > "${initLog}" 2>&1
...
# Start a pre-initialized extra validator and register it in StakeHub.
# Usage: bsc_cluster.sh add-validator <vidx>
#   <vidx> must be in range [BSC_CLUSTER_SIZE, BSC_CLUSTER_SIZE + EXTRA_VALIDATOR_SIZE)
function add_validator() {
    local i=$1
    if [ -z "${i}" ]; then
        echo "Usage: bsc_cluster.sh add-validator <vidx>"
        echo "  vidx range: [${size}, $((size + extra_size)))"
        exit 1
    fi
    if [ ${i} -lt ${size} ] || [ ${i} -ge $((size + extra_size)) ]; then
        echo "Error: vidx ${i} is out of range [${size}, $((size + extra_size)))"
        exit 1
    fi

    datadir="${workspace}/.local/node${i}"
    if [ ! -d "${datadir}/geth/chaindata" ]; then
        echo "Error: node${i} is not initialized. Did you run 'reset' first?"
        exit 1
    fi
    ...
    echo "Node${i} started (cons_addr=${cons_addr}). Waiting 45s before registering in StakeHub..."
    sleep 45
    ${workspace}/create-validator/create-validator \
        ...
        --amount 20001 \
        --validator-desc Val${i} \
        --rpc-url ${RPC_URL}
    echo "Validator ${i} (${cons_addr}) registered in StakeHub."
}
...
restart)
    ...
    ;;
add-validator)
    add_validator $ValidatorIdx
    ;;
*)
    echo "Usage: bsc_cluster.sh reset | stop [vidx] | start [vidx] | restart [vidx] | add-validator <vidx>"
    ;;
esac
