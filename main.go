...
func main() {
	...
	fmt.Printf("[createValidator] consensusAddr=%s\n", consensusAddr.Hex())
	fmt.Printf("[createValidator] chainId=%s\n", chainId.String())
	fmt.Printf("[createValidator] len(pubKey)=%d (contract needs 48)\n", len(pubKey))
	...
	data, err := stakeHubAbi.Pack(method, consensusAddr, pubKey[:], proofBytes, commission, description)
	if err != nil {
		panic(err)
	}
