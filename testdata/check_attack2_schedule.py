...
ROW_RE = re.compile(r"\{(\d+),\s*(expAddr\w+),\s*(nil|\[\]string\{([^}]*)\})\}")
...
def load_schedule(validators_go: Path):
    ...
    rows = []
    for height, miner_name, target_expr, target_body in ROW_RE.findall(text):
        targets = []
        if target_expr != "nil":
...
def main():
    parser = argparse.ArgumentParser(description="Check attack-2 seal schedule against node logs.")
