#!/usr/bin/env python3
"""Build total-difficulty series from BSC attack 2 experiment logs."""

from __future__ import annotations

import argparse
import csv
import re
from dataclasses import dataclass
from pathlib import Path


REPO_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_LOG_ROOT = REPO_ROOT / "node-deploy/.local"
DEFAULT_VALIDATORS_FILE = REPO_ROOT / "code/attack-2-turnlen-8-code/params/validators.go"
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "attack_2_total_difficulty.csv"

SEALED_RE = re.compile(
    r'Successfully seal and write new block".*?\bnumber=(?P<slot>\d+)'
    r".*?\bdifficulty=(?P<difficulty>\d+)"
    r'.*?"total difficulty"=(?P<td>\d+)'
)
NODE_DIR_RE = re.compile(r"node(?P<node>\d+)$")
COMMENT_NODE_RE = re.compile(
    r'"(?P<addr>0x[0-9a-fA-F]+)":\s*"[^"]+",\s*//\s*(?P<node>\d+)\s*(?:\r?\n|$)'
)
VAR_BLOCK_RE = re.compile(
    r"var\s+(?P<name>ValidatorsAddA|ValidatorsAddB|after487LegacyTargetsA|after487LegacyTargetsB)"
    r"\s*=\s*(?:map\[string\]string|(?:\[\]string))\s*\{(?P<body>.*?)\n\}",
    re.S,
)
MAP_ENTRY_RE = re.compile(r'"(?P<addr>0x[0-9a-fA-F]+)":')
CONST_RE = re.compile(r"\b(?P<name>expAddr\w+)\s*=\s*\"(?P<addr>0x[0-9a-fA-F]+)\"")
CONST_REF_RE = re.compile(r"\bexpAddr\w+\b")
# Rows in buildExperimentSchedule: even index -> chain A, odd -> chain B (matches params/validators.go).
SCHEDULE_HEIGHT_ROW_RE = re.compile(
    r"\{heights\((?P<lo>\d+),\s*(?P<hi>\d+)\),\s*(?P<miner>expAddr\w+),"
)
# Avoid matching commented-out lines like `// NetworkSplitManualEnd   uint64 = 490`.
NETWORK_SPLIT_MANUAL_END_RE = re.compile(
    r"^\s*NetworkSplitManualEnd\s+uint64\s*=\s*(?P<end>\d+)", re.M
)


@dataclass(frozen=True)
class SealEvent:
    slot: int
    difficulty: int
    total_difficulty: int
    branch: str | None
    node: str


def normalize(addr: str) -> str:
    return addr.lower()


def parse_network_split_manual_end(text: str, default: int = 487) -> int:
    match = NETWORK_SPLIT_MANUAL_END_RE.search(text)
    if match:
        return int(match.group("end"))
    return default


def parse_manual_chain_miners(
    text: str, constants: dict[str, str], addr_to_node: dict[str, str]
) -> tuple[dict[int, str | None], dict[int, str | None]]:
    """Mirror buildExperimentSchedule rows: even row index -> chain A, odd -> chain B."""
    start = text.find("func buildExperimentSchedule")
    if start == -1:
        return {}, {}
    block = text[start : start + 8000]
    chain_a: dict[int, str | None] = {}
    chain_b: dict[int, str | None] = {}
    for idx, match in enumerate(SCHEDULE_HEIGHT_ROW_RE.finditer(block)):
        lo = int(match.group("lo"))
        hi = int(match.group("hi"))
        miner_const = match.group("miner")
        addr = constants.get(miner_const)
        node: str | None = addr_to_node.get(addr) if addr else None
        target = chain_a if idx % 2 == 0 else chain_b
        for height in range(lo, hi + 1):
            target[height] = node
    return chain_a, chain_b


def parse_validators(validators_file: Path) -> tuple[set[str], set[str], str]:
    """ValidatorsAdd{A,B} plus after487LegacyTargets{A,B} -> post-manual branch node sets (matches Go branchForLateExperimentMiner)."""
    text = validators_file.read_text(encoding="utf-8", errors="replace")
    constants = {m.group("name"): normalize(m.group("addr")) for m in CONST_RE.finditer(text)}
    addr_to_node: dict[str, str] = {}
    for match in COMMENT_NODE_RE.finditer(text):
        # The numeric node list appears before short address aliases such as // 29 or // 511.
        addr_to_node.setdefault(normalize(match.group("addr")), match.group("node"))

    groups: dict[str, set[str]] = {"A": set(), "B": set()}
    for match in VAR_BLOCK_RE.finditer(text):
        name = match.group("name")
        body = match.group("body")
        branch = "A" if name.endswith("A") else "B"
        addresses: set[str] = set()

        if name.startswith("ValidatorsAdd"):
            addresses.update(normalize(m.group("addr")) for m in MAP_ENTRY_RE.finditer(body))
        else:
            for const_name in CONST_REF_RE.findall(body):
                if const_name in constants:
                    addresses.add(constants[const_name])

        for addr in addresses:
            node = addr_to_node.get(addr)
            if node is not None:
                groups[branch].add(node)

    return groups["A"], groups["B"], text


def discover_logs(log_root: Path, log_glob: str) -> list[Path]:
    return sorted(path for path in log_root.glob(f"node*/{log_glob}") if path.is_file())


def node_for_log(path: Path) -> str | None:
    match = NODE_DIR_RE.fullmatch(path.parent.name)
    if not match:
        return None
    return match.group("node")


def branch_for_node(node: str, a_nodes: set[str], b_nodes: set[str]) -> str | None:
    if node in a_nodes:
        return "A"
    if node in b_nodes:
        return "B"
    return None


def parse_seals(
    log_paths: list[Path],
    a_nodes: set[str],
    b_nodes: set[str],
    split_height: int,
    manual_end: int,
    end: int | None,
    td_is_parent: bool,
) -> list[SealEvent]:
    events: list[SealEvent] = []
    for log_path in log_paths:
        node = node_for_log(log_path)
        if node is None:
            continue
        default_branch = branch_for_node(node, a_nodes, b_nodes)
        with log_path.open("r", encoding="utf-8", errors="replace") as handle:
            for line in handle:
                match = SEALED_RE.search(line)
                if not match:
                    continue
                slot = int(match.group("slot"))
                if end is not None and slot > end:
                    continue
                difficulty = int(match.group("difficulty"))
                total_difficulty = int(match.group("td"))
                if td_is_parent:
                    total_difficulty += difficulty
                if slot < split_height:
                    branch = default_branch
                elif slot <= manual_end:
                    # Manual window: branch labels come from scheduled miners in build_rows, not here.
                    branch = None
                else:
                    branch = default_branch
                events.append(
                    SealEvent(
                        slot=slot,
                        difficulty=difficulty,
                        total_difficulty=total_difficulty,
                        branch=branch,
                        node=node,
                    )
                )
    return events


def max_by_slot(events: list[SealEvent], branch: str | None = None) -> dict[int, int]:
    out: dict[int, int] = {}
    for event in events:
        if branch is not None and event.branch != branch:
            continue
        if branch is None and event.branch is not None:
            # Benchmark is taken from pre-split common-chain observations only.
            continue
        prev = out.get(event.slot)
        if prev is None or event.total_difficulty > prev:
            out[event.slot] = event.total_difficulty
    return out


def any_max_by_slot(events: list[SealEvent]) -> dict[int, int]:
    out: dict[int, int] = {}
    for event in events:
        prev = out.get(event.slot)
        if prev is None or event.total_difficulty > prev:
            out[event.slot] = event.total_difficulty
    return out


def value_at(series: dict[int, int], slot: int) -> int | None:
    current: int | None = None
    for key in sorted(series):
        if key > slot:
            break
        current = series[key]
    return current


def max_td_by_slot_node(events: list[SealEvent]) -> dict[tuple[int, str], int]:
    out: dict[tuple[int, str], int] = {}
    for event in events:
        key = (event.slot, event.node)
        prev = out.get(key)
        if prev is None or event.total_difficulty > prev:
            out[key] = event.total_difficulty
    return out


def sparse_manual_series(
    seal_td: dict[tuple[int, str], int],
    miner_by_height: dict[int, str | None],
    lo: int,
    hi: int,
) -> dict[int, int]:
    series: dict[int, int] = {}
    for height in range(lo, hi + 1):
        node = miner_by_height.get(height)
        if not node:
            continue
        td = seal_td.get((height, node))
        if td is not None:
            series[height] = td
    return series


def build_rows(
    events: list[SealEvent],
    start: int,
    end: int,
    split_height: int,
    manual_end: int,
    miner_node_a: dict[int, str | None],
    miner_node_b: dict[int, str | None],
) -> list[dict[str, int | str]]:
    benchmark = any_max_by_slot([event for event in events if event.slot < split_height])
    seal_td = max_td_by_slot_node(events)

    manual_lo = split_height
    manual_hi = min(manual_end, end)
    manual_a_sparse: dict[int, int] = {}
    manual_b_sparse: dict[int, int] = {}
    if manual_lo <= manual_hi:
        manual_a_sparse = sparse_manual_series(seal_td, miner_node_a, manual_lo, manual_hi)
        manual_b_sparse = sparse_manual_series(seal_td, miner_node_b, manual_lo, manual_hi)

    post_a = max_by_slot(events, "A")
    post_b = max_by_slot(events, "B")

    combined_a = {**manual_a_sparse, **{s: td for s, td in post_a.items() if s > manual_end}}
    combined_b = {**manual_b_sparse, **{s: td for s, td in post_b.items() if s > manual_end}}

    benchmark_base_slot = split_height - 1
    benchmark_base_td = value_at(benchmark, benchmark_base_slot)

    rows: list[dict[str, int | str]] = []
    for slot in range(start, end + 1):
        if benchmark_base_td is not None and slot >= split_height:
            benchmark_td = benchmark_base_td + 2 * (slot - benchmark_base_slot)
        else:
            benchmark_td = value_at(benchmark, slot)
        if slot < split_height:
            a_td: int | str | None = 0
            b_td: int | str | None = 0
        else:
            a_td = value_at(combined_a, slot)
            b_td = value_at(combined_b, slot)
            if a_td is None:
                a_td = ""
            if b_td is None:
                b_td = ""
        rows.append(
            {
                "slot": slot,
                "benchmark_total_difficulty": "" if benchmark_td is None else benchmark_td,
                "branch_a_total_difficulty": a_td,
                "branch_b_total_difficulty": b_td,
            }
        )
    return rows


def write_csv(rows: list[dict[str, int | str]], output: Path) -> None:
    output.parent.mkdir(parents=True, exist_ok=True)
    with output.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=[
                "slot",
                "benchmark_total_difficulty",
                "branch_a_total_difficulty",
                "branch_b_total_difficulty",
            ],
        )
        writer.writeheader()
        writer.writerows(rows)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze attack 2 total difficulty from node logs.")
    parser.add_argument("--log-root", type=Path, default=DEFAULT_LOG_ROOT)
    parser.add_argument("--log-glob", default="bsc.log*")
    parser.add_argument("--validators", type=Path, default=DEFAULT_VALIDATORS_FILE)
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--start", type=int, default=300)
    parser.add_argument("--end", type=int, default=440)
    parser.add_argument("--split-height", type=int, default=398)
    parser.add_argument(
        "--manual-end",
        type=int,
        default=None,
        help="Last height of buildExperimentSchedule manual window (default: NetworkSplitManualEnd in validators.go).",
    )
    parser.add_argument(
        "--td-is-parent",
        action="store_true",
        help="Treat logged total difficulty as parent TD and add current block difficulty.",
    )
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    a_nodes, b_nodes, validators_text = parse_validators(args.validators)
    manual_end = args.manual_end if args.manual_end is not None else parse_network_split_manual_end(validators_text)
    constants = {
        m.group("name"): normalize(m.group("addr"))
        for m in CONST_RE.finditer(validators_text)
    }
    addr_to_node: dict[str, str] = {}
    for match in COMMENT_NODE_RE.finditer(validators_text):
        addr_to_node.setdefault(normalize(match.group("addr")), match.group("node"))
    miner_node_a, miner_node_b = parse_manual_chain_miners(validators_text, constants, addr_to_node)

    log_paths = discover_logs(args.log_root, args.log_glob)
    events = parse_seals(
        log_paths,
        a_nodes,
        b_nodes,
        args.split_height,
        manual_end,
        args.end,
        args.td_is_parent,
    )
    rows = build_rows(
        events,
        args.start,
        args.end,
        args.split_height,
        manual_end,
        miner_node_a,
        miner_node_b,
    )
    write_csv(rows, args.output)
    print(f"wrote {len(rows)} rows to {args.output}")
    print(f"logs={len(log_paths)} seal_events={len(events)}")
    print(f"split_height={args.split_height} manual_end={manual_end}")
    print(f"branch_a_nodes={sorted(a_nodes)} branch_b_nodes={sorted(b_nodes)}")
    sched_pos = validators_text.find("func buildExperimentSchedule")
    sched_block = validators_text[sched_pos : sched_pos + 8000] if sched_pos != -1 else ""
    print(f"buildExperimentSchedule_rows={sum(1 for _ in SCHEDULE_HEIGHT_ROW_RE.finditer(sched_block))}")


if __name__ == "__main__":
    main()
